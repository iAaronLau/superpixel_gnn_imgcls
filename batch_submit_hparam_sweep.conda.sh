...
DATASETS=${DATASETS:-cifar10,imagewoof}
...
BATCH_SIZE_GRID=${BATCH_SIZE_GRID:-16,32,64}
RESNET_BATCH_SIZE_GRID=${RESNET_BATCH_SIZE_GRID:-64,128,256}
GRAPH_BATCH_SIZE_GRID=${GRAPH_BATCH_SIZE_GRID:-16,32,64}
WEIGHT_DECAY_GRID=${WEIGHT_DECAY_GRID:-1e-4}

SEED=${SEED:-42}
CIFAR_EPOCHS=${CIFAR_EPOCHS:-50}
IMAGEWOOF_EPOCHS=${IMAGEWOOF_EPOCHS:-20}
...
MAX_GRAPH_BATCH_CIFAR=${MAX_GRAPH_BATCH_CIFAR:-64}
MAX_GRAPH_BATCH_IMAGEWOOF=${MAX_GRAPH_BATCH_IMAGEWOOF:-16}
...
BUILD_CMD=(
  ...
  --resnet_batch_size_grid "$RESNET_BATCH_SIZE_GRID"
  --graph_batch_size_grid "$GRAPH_BATCH_SIZE_GRID"
...
SBATCH_CMD=(
  ...
  --error "$SBATCH_ERROR"
  --export "ALL,PROJECT_ROOT=$PROJECT_ROOT,MANIFEST=$MANIFEST_PATH,SWEEP_ROOT=$SWEEP_ROOT,PYTHON_BIN=$PYTHON_BIN,RETRIES=$RETRIES,RETRY_SLEEP_SEC=$RETRY_SLEEP_SEC,SKIP_IF_RECORDED=$SKIP_IF_RECORDED,AUTO_CONDA_ACTIVATE=$AUTO_CONDA_ACTIVATE_TASK"
)
...
echo "[Submit] batch_grid resnet=$RESNET_BATCH_SIZE_GRID graph=$GRAPH_BATCH_SIZE_GRID"
