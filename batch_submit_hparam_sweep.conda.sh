...
DATASETS=${DATASETS:-imagenette2,imagewoof,cifar10}
...
LARGE_IMAGE_GRAPH_SEGMENTS_GRID=${LARGE_IMAGE_GRAPH_SEGMENTS_GRID:-100,200,300}
GRAPH_HIDDEN_DIM_GRID=${GRAPH_HIDDEN_DIM_GRID:-256,384}
GRAPH_LAYERS_GRID=${GRAPH_LAYERS_GRID:-4}
GRAPH_POOLING_GRID=${GRAPH_POOLING_GRID:-meanmax}
GRAPH_DROPOUT_GRID=${GRAPH_DROPOUT_GRID:-0.1,0.2}
GAT_HEADS_GRID=${GAT_HEADS_GRID:-4,8}
GRAPH_COMPACTNESS_GRID=${GRAPH_COMPACTNESS_GRID:-10,20}
...
GRAPH_CACHE_VERSION=${GRAPH_CACHE_VERSION:-v3}
GRAPH_CACHE_LOCK_TIMEOUT_SEC=${GRAPH_CACHE_LOCK_TIMEOUT_SEC:-30}
GRAPH_CACHE_FALLBACK_TO_LOCAL=${GRAPH_CACHE_FALLBACK_TO_LOCAL:-1}
GRAPH_CACHE_WAIT_LOG_EVERY_SEC=${GRAPH_CACHE_WAIT_LOG_EVERY_SEC:-60}
GRAPH_CACHE_STALE_LOCK_SEC=${GRAPH_CACHE_STALE_LOCK_SEC:-7200}
...
SPGNN_DISABLE_GRAPH_CUDAGRAPHS=${SPGNN_DISABLE_GRAPH_CUDAGRAPHS:-1}
...
BUILD_CMD=(
  ...
  --large_image_graph_segments_grid "$LARGE_IMAGE_GRAPH_SEGMENTS_GRID"
  --graph_hidden_dim_grid "$GRAPH_HIDDEN_DIM_GRID"
  --graph_layers_grid "$GRAPH_LAYERS_GRID"
  --graph_pooling_grid "$GRAPH_POOLING_GRID"
  --graph_dropout_grid "$GRAPH_DROPOUT_GRID"
  --gat_heads_grid "$GAT_HEADS_GRID"
  --graph_compactness_grid "$GRAPH_COMPACTNESS_GRID"
...
SBATCH_CMD=(
  ...
  --error "$SBATCH_ERROR"
  --export "ALL,PROJECT_ROOT=$PROJECT_ROOT,MANIFEST=$MANIFEST_PATH,SWEEP_ROOT=$SWEEP_ROOT,PYTHON_BIN=$PYTHON_BIN,RETRIES=$RETRIES,RETRY_SLEEP_SEC=$RETRY_SLEEP_SEC,SKIP_IF_RECORDED=$SKIP_IF_RECORDED,IDLE_TIMEOUT_SEC=$IDLE_TIMEOUT_SEC,AUTO_CONDA_ACTIVATE=$AUTO_CONDA_ACTIVATE_TASK,GRAPH_CACHE_LOCK_TIMEOUT_SEC=$GRAPH_CACHE_LOCK_TIMEOUT_SEC,GRAPH_CACHE_FALLBACK_TO_LOCAL=$GRAPH_CACHE_FALLBACK_TO_LOCAL,GRAPH_CACHE_WAIT_LOG_EVERY_SEC=$GRAPH_CACHE_WAIT_LOG_EVERY_SEC,GRAPH_CACHE_STALE_LOCK_SEC=$GRAPH_CACHE_STALE_LOCK_SEC,SPGNN_DISABLE_GRAPH_CUDAGRAPHS=$SPGNN_DISABLE_GRAPH_CUDAGRAPHS"
)
